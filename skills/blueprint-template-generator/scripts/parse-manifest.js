...
function getManifestDir() {
  ...
  // Check if we're in a skill directory (manifests bundled with skill)
  if (existsSync(join(skillRoot, 'blueprints', 'manifests'))) {
    return join(skillRoot, 'blueprints', 'manifests');
  }
...
export function loadManifest(blueprintName) {
  ...
  try {
    ...
    if (!manifest.name || !manifest.snippets) {
