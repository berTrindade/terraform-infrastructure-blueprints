---
name: infrastructure-style-guide
description: Use when selecting blueprints, writing Terraform for blueprint-based infra, or making architectural decisions. Covers security, cost, naming, and code quality by priority (CRITICAL, HIGH, MEDIUM, LOW).
---
...
- [ ] Have I checked Skills for static content? (`infrastructure-style-guide`)
...
**For adding capabilities to existing projects**: Use `infrastructure-code-generation` skill to generate code locally:
...
**Decision**: 
- **"Add capability"** → Use `infrastructure-code-generation` skill
...
3. **Use `infrastructure-code-generation` skill** to generate RDS module code:
...
- `infrastructure-style-guide`: This consolidated skill (catalog, patterns, workflows)
- `infrastructure-code-generation`: Generate code from templates (for adding capabilities)
