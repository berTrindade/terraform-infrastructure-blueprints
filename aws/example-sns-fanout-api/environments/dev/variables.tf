...
# Development environment input variables (API Gateway → SNS → Subscribers)
...
# ============================================
# SQS Configuration (shared by all subscribers)
# ============================================

variable "sqs_retention_seconds" {
  description = "Message retention in subscriber queues (seconds)"
  type        = number
  default     = 86400 # 1 day
...
variable "notifier_filter_policy" {
  description = "SNS filter policy for notifier (null for all events)"
  type        = map(any)
  default     = null
  # Example: { "eventType": ["ReportRequested", "ReportGenerated"] }
}
