...
4. Run script: `cd skills/infrastructure-code-generation && node scripts/generate.js` with payload on stdin
...
The skill executes:
```bash
cd skills/infrastructure-code-generation
...
- Check template file exists: `skills/infrastructure-code-generation/templates/{template-name}` (e.g. `rds-module.tftpl`)
...
- **infrastructure-style-guide**: Catalog, patterns, workflow context, and when to use MCP vs this skill
